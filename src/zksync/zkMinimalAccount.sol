 ...
 * msg.sender is the bootloader(think of it as a super admin) system contract
 *
 * Phase 1 Validation
 * 1. The user sends the validation to zkSync API client
 * 2. The zkSync API client checks to see if the nonce is unique by querying the Nonceholder system contract
 * 3. The zkSync API client calls validTransaction, which MUST update the nonce
 * 4. The zkSync API client checks if the nonce is updated
 * 5. The zkSync API client calls payForTransaction, or prepareForPaymaster & validateAndPayForPaymasterTransaction
 * 6. The zkSync API client varifies that the bootloader gets paid
 *
 * Phase 2 Execution
 * 7. The zkSync API client passes the validated transaction to the main node / sequencer (as of today they are both the same)
...
contract ZkMinimalAccount is IAccount, Ownable {
    ...
    /**
     * @notice must increase the none
     * @notice must validate the transaction (check the owner signed transaction)
